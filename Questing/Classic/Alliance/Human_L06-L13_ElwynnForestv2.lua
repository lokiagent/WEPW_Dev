...
function PreferredVendor()
    VendorName = "Corina Steele"; VendorX = -9463.543; VendorY = 93.18085; VendorZ = 58.3259;
...
while Player.Level >= 7 and Player.Level < 9 do
    ...
    GrindAndGather(TableToList{113}, 50, TableToFloatArray({-9946.781, 90.59814, 32.97921}), TrainSkinning);
end
...
if HasPlayerFinishedQuest(107) and HasPlayerFinishedQuest ~= true then
    GoToTrainer(ClassTrainer, ClassTrainerX, ClassTrainerY, ClassTrainerZ); Log("Go to Trainer: "..ClassTrainer);
...
----Manual Sell, figure logic out later
...
while Player.Level >= 9 and Player.Level < 12 do
    ...
    GrindAndGather(TableToList(RiverpawGnolls), 200, TableToFloatArray({-8989.872, -764.0568, 74.30352}), false);
end
...
while (CanTurnInQuest(11) ~= true) do
    GrindAndGather(TableToList(RiverpawGnolls), 200, TableToFloatArray({-8989.872, -764.0568, 74.30352}), false); Log("Completing Objective: [10]Riverpaw Gnoll Bounty");
end
